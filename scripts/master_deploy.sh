...
# Функция ожидания блокировки apt
wait_for_apt_lock() {
    print_info "Ожидание освобождения блокировки apt (dpkg/lock-frontend)..."
    while fuser /var/lib/dpkg/lock-frontend >/dev/null 2>&1 || fuser /var/lib/apt/lists/lock >/dev/null 2>&1 || fuser /var/lib/dpkg/lock >/dev/null 2>&1 ; do
        print_info "Блокировка все еще удерживается другим процессом (вероятно, фоновое обновление)... ждем 5 секунд..."
        sleep 5
    done
}
