...
- ОПАСНОСТЬ: Никогда не используй символ `_` (нижнее подчеркивание) просто так в тексте или названиях, так как это сломает форматирование Telegram. Если в названии товара есть `_`, замени его на пробел или дефис.
- Удаляй пробелы в конце названий (например, "Жилетка " -> "Жилетка").
- Всегда оборачивай ссылки на товары в двойные звездочки **[текст](ссылка)**, чтобы они были жирными.
