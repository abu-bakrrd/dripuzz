...
print("🚀 ЗАПУСК БОТА: ВЕРСИЯ 4.6 (PRETTY RESPONSES)", flush=True)
...
class AICustomerBot:
    ...
    def __init__(self, bot_token, gemini_key):
        ...
        self.SPAM_LIMIT = 7  # Максимум сообщений
...
- **Внутренний (Мысли)**: Вызывай их для получения RAW данных.
- **Внешний (Ответ)**: Если ты хочешь ПОКАЗАТЬ клиенту красивую карточку или статус заказа, вставь тег инструмента прямо в свой финальный текст. Система автоматически заменит его на красивый оформленный блок.

#### 1. ТВОИ ИНСТРУМЕНТЫ:
- **`[ПОИСК:запрос]`**: Твои глаза. Находит ID и названия.
- **`[ИНФО:id]`**: Твоя экспертиза. Если ты вставишь этот тег в ответ клиенту, он увидит роскошную карточку товара с ценой, описанием и таблицей размеров.
- **`[ТОВАРЫ:старт,стоп]`**: Твоя витрина. Всегда вставляй в ответ на общие вопросы о наличии.
- **`[ЗАКАЗ:id]`**: Твоя ответственность. Вставь в ответ, чтобы клиент увидел детальный статус своего заказа.
- **`[КАТАЛОГ]`**: Твоя память для исправления опечаток.

#### 2. СЕКРЕТ КРАСИВОГО ОТВЕТА:
Если клиент спросил детали о пальто, твой идеальный ответ:
"О, это прекрасный выбор! Вот все подробности по этой модели:\n\n[ИНФО:id]"
Система сама сделает всё красиво. Тебе не нужно переписывать размеры или состав руками из DATABASE_RAW_DATA, если ты используешь тег [ИНФО:id] в финальном ответе.
...
Меня зовут <b>Mona</b>, и я твой AI-консультант магазина Monvoir! ✨ (v4.6)
...
"""
    ...
    def run(self):
        ...
        print("🤖 AI Customer Bot запущен и готов к работе (v4.6 STABLE)...")
