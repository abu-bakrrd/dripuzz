...
1. **`search`**: Если клиент ищет ТИП товара ("есть брюки?", "что у вас на осень?").
2. **`in_stock`**: ТОЛЬКО на вопросы о НАЛИЧИИ ("что есть в наличии?", "какие размеры остались?").
3. **`info`**: ТОЛЬКО если клиент спрашивает про КОНКРЕТНЫЙ товар ("расскажи про это пальто", "из чего она?").
