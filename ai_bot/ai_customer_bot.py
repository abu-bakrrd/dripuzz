...
- Используй эмодзи часто для оживления текста: ✨, 👔, 💼, 📦, 🛍️, ✅, 🔍, 💕, 💖, 🌟, 💫
- Будь внимательной к деталям и заботливой к клиентам.
- Проявляй интерес к их потребностям, но без навязчивости.

КРИТИЧЕСКИ ВАЖНО - НЕ ПРИДУМЫВАЙ:
- ⛔️ НИКОГДА не выдумывай товары, которых нет в предоставленной информации.
- ⛔️ НИКОГДА не придумывай цены, цвета, размеры, характеристики.
- ⛔️ НИКОГДА не говори о товарах, которых нет в списке "ТОВАРЫ В МАГАЗИНЕ".
- ✅ Говори ТОЛЬКО о товарах, которые указаны в разделе "ТОВАРЫ В МАГАЗИНЕ".
- ✅ Если товара нет в списке - скажи, что его нет или предложи поиск на <a href="https://monvoir.shop">сайте</a>.
- ✅ Если клиент спрашивает о товаре, которого нет в списке - честно скажи: "К сожалению, этого товара нет в моем списке. Попробуйте найти его на <a href="https://monvoir.shop">нашем сайте</a> или используйте команду /search."

О ТОВАРАХ:
- Ты видишь только <b>небольшую часть</b> товаров (обычно 3-5 примеров).
- Если клиент просит "еще", "другие", "покажи еще товары" - скажи, что можешь показать больше товаров, но нужно уточнить запрос или поискать на <a href="https://monvoir.shop">сайте</a>.
- Всегда упоминай, что на <a href="https://monvoir.shop">сайте</a> больше товаров, если клиент ищет что-то конкретное.

ПРАВИЛА ОБЩЕНИЯ:
- Отвечай ТОЛЬКО на заданные вопросы, без лишнего. Будь лаконичной, но дружелюбной.
...
НАВИГАЦИЯ: Главная (каталог, фильтры, поиск товаров), Карточка товара (выбор цвета/размера, кнопка "Добавить в корзину"), Корзина (просмотр уже выбранных товаров, изменение количества, оформление заказа), Избранное (сохраненные товары), Профиль (история заказов). ВАЖНО: Корзина - это место, где уже находятся товары, которые пользователь добавил. НЕ говори "корзина для покупок" - это нелогично. Ссылки: <a href="https://monvoir.shop">сайт</a>, <a href="https://monvoir.shop/cart">корзина</a>.
...
ФОРМАТИРОВАНИЕ - ОБЯЗАТЕЛЬНО ЧАСТОЕ ИСПОЛЬЗОВАНИЕ HTML:
...
- ✅ Используй <b>ОЧЕНЬ ЧАСТО</b> для: цен (<b>450,000 сум</b>), названий товаров (<b>Название</b>), важных слов (<b>есть в наличии</b>), ключевых фраз
- ✅ Используй <i>ОЧЕНЬ ЧАСТО</i> для: описаний (<i>Идеально для офиса</i>), уточнений (<i>Есть в наличии</i>), дополнительной информации
- ✅ Комбинируй теги: <a href="url"><b>Название</b></a>, <i><b>Особое предложение</b></i>
- ✅ Все ссылки ТОЛЬКО в формате <a href="url"><b>текст</b></a>, НИКОГДА не показывай открытые URL
- ✅ Эмодзи используй часто: ✨, 👔, 💼, 📦, 🛍️, ✅, 🔍, 💕, 💖, 🌟, 💫
- ✅ Цвета: названия на русском (не HEX). ТОЧНОСТЬ: используй точные названия из базы, не выдумывай. Без thinking tags.
"""
        ...
        def handle_question(message):
            ...
            try:
                ...
                try:
                    ...
                    # Если запрос "еще" - используем предыдущий поиск из истории или показываем больше товаров
                    if is_more_request:
                        # Пытаемся найти предыдущий запрос в истории
                        previous_query = None
                        if session['history']:
                            # Ищем последний вопрос пользователя о товарах
                            for msg in reversed(session['history'][-10:]):
                                if msg['role'] == 'user':
                                    prev_text = msg['text'].lower()
                                    # Если это был вопрос о товарах (не команда и не про заказ)
                                    if not prev_text.startswith('/') and not any(word in prev_text for word in ['заказ', 'статус', 'номер']):
                                        # Убираем слова "еще", "другие" и берем оригинальный запрос
                                        clean_query = prev_text
                                        for word in more_keywords:
                                            clean_query = clean_query.replace(word, '').strip()
                                        if clean_query:
                                            previous_query = clean_query
                                            break
                        
                        # Если нашли предыдущий запрос - ищем больше товаров
                        if previous_query:
                            # Ищем товары по предыдущему запросу
                            search_results = search_products(previous_query)
                            if search_results:
                                # Показываем больше товаров (до 6 вместо 3)
                                found_products_list = search_results[:6]
                            else:
                                # Если поиск не дал результатов, показываем больше примеров
                                all_products = get_all_products_info()
                                found_products_list = all_products[:6] if all_products else []
                        else:
                            # Если не нашли предыдущий запрос - показываем больше примеров
                            all_products = get_all_products_info()
                            found_products_list = all_products[:6] if all_products else []
                        
                        if found_products_list:
                            products_context = format_products_for_ai(found_products_list)
                            products_context += "\n\n<i>Это больше товаров из каталога. На <a href=\"https://monvoir.shop\">сайте</a> вы найдете полный ассортимент с фильтрами и поиском.</i>"
                        else:
                            products_context = "ТОВАРЫ В МАГАЗИНЕ:\n\nБольше товаров не найдено. Попробуйте поискать на <a href=\"https://monvoir.shop\">нашем сайте</a>."
                    else:
                        # Обычный поиск
                        ...
                        if found_products_list:
                            # Ограничиваем до 3 товаров для первого запроса
                            is_search_query = True
                            products_context = format_products_for_ai(found_products_list[:3])
                            # Добавляем информацию о том, что товаров может быть больше
                            total_found = len(found_products_list)
                            if total_found > 3:
                                products_context += f"\n\n<i>Найдено товаров: {total_found}. Показано 3. Если нужно больше - напишите \"еще\" или \"покажи еще\".</i>"
                            else:
                                products_context += f"\n\n<i>На <a href=\"https://monvoir.shop\">сайте</a> вы найдете больше товаров с фильтрами и поиском.</i>"
                        elif is_general:
                            ...
                            if found_products_list:
                                ...
                                products_context += f"\n\n<i>Показано 3 примера из {total_products} товаров. Напишите \"еще\" или \"покажи еще\" чтобы увидеть больше. На <a href=\"https://monvoir.shop\">сайте</a> полный каталог.</i>"
                            else:
                                products_context = "ТОВАРЫ В МАГАЗИНЕ:\n\nВ каталоге пока нет товаров."
                        else:
                            # Конкретный запрос, но ничего не найдено
                            products_context = "ТОВАРЫ В МАГАЗИНЕ:\n\nТовар не найден в каталоге. Попробуйте поискать на <a href=\"https://monvoir.shop\">нашем сайте</a> или используйте команду /search для поиска по фото."
                except Exception as e:
